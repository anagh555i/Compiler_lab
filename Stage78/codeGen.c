...
int handleFunctionReturn(node* root){
    ...
    fprintf(outFile,"MOV [R%d], R%d\n",r1,reg);
    Lsymbol* lvar;
    if(currFunction) lvar=currFunction->fsymbols;
    else if(currMethod) lvar=currMethod->fsymbols;
...
int handleMethodcall(node* root){
    int i;
    for(i=0;i<currReg;i++) fprintf(outFile,"PUSH R%d\n",i); // registers in use;
    int thisAddress=push_reverse_methods(root->left); // push space for arguments
    fprintf(outFile,"PUSH R0\n"); // space for return value;
...
void calculateMethodAddress(int reg,int methodNo){
    fprintf(outFile,"MOV R%d, [R%d]\n",reg,reg);
    fprintf(outFile,"BRKP\n");
    fprintf(outFile,"MUL R%d, 8\n",reg);
    fprintf(outFile,"ADD R%d, %d\n",reg,methodNo);
    fprintf(outFile,"ADD R%d, %d\n",reg,vFunctTableStart);
    fprintf(outFile,"MOV R%d, [R%d]\n",reg,reg);
}

int push_reverse_methods(node* root){
    if(root==NULL) return getReg();
    int argval=push_reverse_methods(root->center);
    freeReg();
...
void push_reverse(node* root){
    ...
    int reg=genCode(root);
    // handleWrite(reg);
    printf("this method call atleast");
    // handleWrite(reg);
    fprintf(outFile,"PUSH R%d\n",reg);
    freeReg();
}
...
int handleNew(node* root){
    int reg=handleMalloc(root);
    fprintf(outFile,"MOV [R%d], %d\n",reg,root->ptrType->classIndex);
    return reg;
}
