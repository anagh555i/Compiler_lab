...
int calculateAddress(node* root){
    ...
    Lnode* x=root->var->dim->next; // size of each index in chain
    int size=root->var->size;
    int r1=getReg();
    int r2;
    fprintf(outFile,"MOV R%d, %d\n",r1,size);
    fprintf(outFile,"MOV R%d, %d\n",reg,root->var->binding);
    ...
    r2=getReg();
    fprintf(outFile,"MOV R%d, %d\n",r1,size);
    fprintf(outFile,"ADD R%d, %d\n",r1,root->var->binding); //r1 contians limit of arr size
    ...
    fprintf(outFile,"JNZ R%d, ERRORMESS!",r1);
...
void handleAssignment(node* root){
    ...
    node* ptr;
    // if(!root->left) printf("No root->left\n");
    // if(!root->left->var) printf("No root->left->var\n");
    // printf("------------%d\n",root->left->var->binding);
    // printf("------------%d\n",root->left->nodeType);
    switch(root->left->nodeType){
        case tVAR:
            fprintf(outFile,"MOV [%d], R%d\n",root->left->var->binding,reg);
        break;
        case tARRVAL:
