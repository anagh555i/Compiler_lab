...
sourceCode      :       GDelcarations FDefBlock MainBlock {root=$2;}
                ;
...
ParamList   : Param ',' ParamList   {} 
            ...
            ;
Param       : Type ID               {
                                        if($1 != inttype || $1!=strtype) yyerror("forbidden type for paramteres");
...
Fdef        : Type ID '(' ParamList ')''{' LdeclBlock Body '}'  {/*iterate through paramlist and Lhead and check correctness and assign relative binding*/
                                                                    ...
                                                                    installFunction(makeNode(0,inttype,funct,funct->paramlist,tFUNCT,NULL,$8,NULL)); //->right points to function definition and left to next function 
                                                                }
            ;
...
MainBlock        : VOID MAIN '('')''{' LdeclBlock Body '}'  {
                                                                ...
                                                                installFunction(makeNode(0,inttype,Ghead,Ghead->paramlist,tFUNCT,NULL,$7,NULL));
                                                            }
            ;
...
Expr    :   Expr '+' Expr           {if(isArithmetic($1,$3)) $$=makeNode(0,inttype,NULL,NULL,tADD,$1,$3,NULL);}
        ...
        |  '&' ID                   {
                                    ...
                                    if(Lptr==NULL && Gptr==NULL) yyerror("undelcared variable");
                                    node* var=makeNode(0,(Lptr?Lptr->type:Gptr->type),Gptr,Lptr,tVAR,NULL,NULL,NULL);
                                    $$=makeNode(0,inttype,NULL,NULL,tADDRESSOF,var,var,var) ; // i don't see the point of address type, now we can do &var+1 and all
                                    }
        |  '*' ID                   {
                                    Gsymbol* Gptr=lookUpG($2);
                                    Lsymbol* Lptr=lookUpL($2);
...
void functionPreperation(node* funct){
    /* //////////////////////////////// */
}
